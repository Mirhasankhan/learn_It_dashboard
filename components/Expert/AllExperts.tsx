...
import { useGetAllExpertsQuery } from "@/redux/api/expertApi";
import ExpertDetailsModal from "../modal/ExpertViewModal";
...
const AllExperts = () => {
  ...
  const {
    ...
    isLoading,
    isFetching,
  } = useGetAllExpertsQuery({ page, searchTerm: searchTerm || "" });
  ...
  return (
    <div className="p-5 mb-6 bg-white rounded-xl">
      <div className="flex flex-col lg:flex-row lg:justify-between lg:items-center gap-4 mb-4">
        <h1 className="md:font-medium">Expert List</h1>
                ...
                <TableRow key={user.id}>
                  ...
                  <TableCell>
                    <div className="flex gap-2">
                      <Link href={`/expert-management/${user.id}`}>
                        {" "}
                        <button className="bg-purple-50 py-2 text-purple-600 px-3 rounded-[6px] cursor-pointer">
                          Earning History
                        </button>
                      </Link>
                      <ExpertDetailsModal userId={user.id} />
                      ...
                      {user.status === "DEACTIVATE" ? (
                        ...
                        <button
                          ...
                          onClick={() => handleDeactivate(user.id)}
                          className="bg-red-100 text-xs disabled:cursor-not-allowed text-red-600 px-4 py-1 rounded-[6px]"
                        >
                          {isDeactivating && isCurrent
                            ? "Deactivating.."
                            : "Deactivate"}
                      ...
                      {new Date(user.suspendUntil) > new Date() ? (
                        <button
                          ...
                          className="bg-purple-50 text-xs text-purple-400 px-4 py-1 rounded-[6px]"
                        >
