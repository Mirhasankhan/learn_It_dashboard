...
import ViewExpertDetailsModal from './ViewExpertDetailsModal';
import { useDeactivateUserMutation, useDeleteUserMutation, useSuspendUserMutation } from '@/redux/api/jobSeekerApi';
...
const ExpertManagement = () => {
    ...
    const { data, isLoading } = useGetAllExpertsQuery({ currentPage, searchTerm });
    ...
    const formatedData = data?.result?.experts?.map((entry: any) => ({
        ...
        Status: (entry?.status === "selected" ?
            <div className='text-[#53C31B] text-xs bg-[#EEF9E8] rounded-lg flex justify-center w-fit px-2 py-1'>Selected</div> :
        ...
        Action: (
            <div className='flex items-center gap-4'>
                <Dialog>
                    ...
                    <ViewExpertDetailsModal id={entry?.id} />
                </Dialog>
                <AlertDialog>
    ...
    return (
        <div>
            <div className='flex flex-col md:flex-row md:justify-between items-center gap-6'>
                <h1 className='text-[#2D2D2D] text-2xl font-semibold'>Expert Management</h1>
