...
export function UpdateTermCondition({type}:any) {
  const [open, setOpen] = useState(false);
  const [content, setContent] = useState("");

  const { data: terms } = useGetAllTermsAndConditionsQuery(type);
  const [updateTerm, { isLoading }] = useUpdateTermsMutation();
  ...
  const handleUpdateTerms = async () => {
    ...
    try {
      const response = await updateTerm(data).unwrap();
